...
SPAWN_AREA = (20, 40, 200, 530)  # x_min, x_max, y_min, y_max, zona donde aparecen inicialmente l34eos agentes
SPAWN_TIME = (10000, 15000) # Se ajusta el tiempo que duran las personas en medir la temperatura antes de entrar
        ...
        while running:
            ...
            if (sim_time - last_spawn_time) > (current_spawn_interval / 1000.0) and len(people) < NUM_PEOPLE:
                ...
                for _ in range(spawn_count):
                    ...
                    # Mask use logic
                    mask = random.random() < MASK_USE # Only 80% of the people use it
                    if mask:
                    ...
                    if infected:
                        if random.random() < TEMP_DETEC: # If the person is infected continue to the next person in line until max people
                            print("persona infectada detectada")
                            continue
...
personas = [10, 15, 25] # Cantidad de personas que se van a simular
...
for porcentaje in porcentajes:
    for persona in personas:
        for distancia in distancias:
            simulation_name = "logic_simulation_p" + str(persona) + "_d" + str(distancia) + "_ir" + str(porcentaje)
